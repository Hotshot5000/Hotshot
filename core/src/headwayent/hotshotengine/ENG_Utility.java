 ...
 * Last modified 11/24/21, 7:57 PM
 * Copyright (c) 2023.
 * All rights reserved.
 */
...
import headwayent.hotshotengine.renderer.nativeinterface.pipeline.ENG_ByteBufferRing;
...
public class ENG_Utility {
    ...
    public static final NumberFormat FORMATTER_DEFAULT_INT = new DecimalFormat("#0000");
...
//    }
    ...
    public static void createRandomNumberGenerator() {
        long seed;
//        System.out.println(ENG_Utility.class.getClassLoader());
...
//			buffer.flip();
    ...
    public static ByteBuffer wrapBuffer(byte[] b, boolean lwjglFix) {
        ...
        int len;
        if (lwjglFix) {
            len = Math.max(b.length, LWJGL_LIMIT);
        } else {
            len = b.length;
        }
...
//		buffer.flip();
    ...
    public static IntBuffer getIntArrayAsBuffer(ENG_Integer[] list) {
        ...
        IntBuffer buffer =
                allocateDirect(len * ENG_Integer.SIZE_IN_BYTES).asIntBuffer();
        for (ENG_Integer engInteger : list) {
            buffer.put(engInteger.getValue());
        }
        return buffer;
    }
    ...
    public static ShortBuffer getShortArrayAsBuffer(ENG_Short[] list) {
        ...
        ShortBuffer buffer =
                allocateDirect(len * ENG_Short.SIZE_IN_BYTES).asShortBuffer();
        for (ENG_Short engShort : list) {
            buffer.put(engShort.getValue());
        }
        return buffer;
    }
    ...
    public static ByteBuffer getByteArrayAsBuffer(ENG_Byte[] list) {
        ...
        ByteBuffer buffer =
                allocateDirect(len * ENG_Byte.SIZE_IN_BYTES);
        for (ENG_Byte engByte : list) {
            buffer.put(engByte.getValue());
        }
        return buffer;
    }
    ...
    public static ByteBuffer getBooleanArrayAsBuffer(
        ...
        ByteBuffer buffer =
                allocateDirect(len * ENG_Byte.SIZE_IN_BYTES);
        for (ENG_Boolean engBoolean : list) {
            buffer.put((byte) (engBoolean.getValue() ? 1 : 0));
        }
        return buffer;
    }
    ...
    public static LongBuffer getLongArrayAsBuffer(
        ...
        LongBuffer buffer =
                allocateDirect(len * ENG_Long.SIZE_IN_BYTES).asLongBuffer();
        for (ENG_Long engLong : list) {
            buffer.put(engLong.getValue());
        }
        return buffer;
    }
    ...
    public static FloatBuffer getFloatArrayAsBuffer(
        ...
        FloatBuffer buffer =
                allocateDirect(len * ENG_Float.SIZE_IN_BYTES).asFloatBuffer();
        for (ENG_Float engFloat : list) {
            buffer.put(engFloat.getValue());
        }
        return buffer;
    }
    ...
    public static DoubleBuffer getDoubleArrayAsBuffer(
        ...
        DoubleBuffer buffer =
                allocateDirect(len * ENG_Double.SIZE_IN_BYTES).asDoubleBuffer();
        for (ENG_Double engDouble : list) {
            buffer.put(engDouble.getValue());
        }
        return buffer;
    }
    ...
    // It happens that we only support power of two textures
    @Deprecated
    public static int getWidthAndHeight(int width, int height) {
        if (width > height) {
            return GLPixelUtil.optionalPO2(width);
        }
        return GLPixelUtil.optionalPO2(height);
    }
    ...
    public static String extractThreadNameWithStacktrace() {
        ...
        return Thread.currentThread().getName() + " " + sb;
    }
    ...
    /**
     *
     * @return the adjusted current time without the time that the application has been
     * in background.
     */
    public static long currentTimeMillis() {
        return System.currentTimeMillis() - MainApp.getMainThread().getPauseTimeMillis();
    }

    /**
     *
     * @return the current system time with no adjustments.
     */
    public static long currentTimeMillisReal() {
        return System.currentTimeMillis();
    }

    /**
     *
     * @return the current system time in nanoseconds with no adjustments.
     */
    ...
    public static boolean calculateCollisionPosition(
            ...
            float projectileSpeed, ENG_Matrix4 inverseWorldMatrix,
        ...
        float a = finalEntityVelocity.dotProduct(finalEntityVelocity)
                - ENG_Math.sqr(projectileSpeed);
        float b = 2.0f * finalEntityVelocity.dotProduct(finalTargetPos4D);
        float c = finalTargetPos4D.dotProduct(finalTargetPos4D);
        ...
        float q = ENG_Math.sqrt(result) / (2 * a);
    ...
    public static void printAngleAxisDeg(ENG_Quaternion q) {
        System.out.println(toAngleAxisDeg(q));
    }

    public static void printAngleAxisDeg(ENG_Quaternion q, String prefix) {
        System.out.println(toAngleAxisDeg(q, prefix));
    }

    public static String toAngleAxisDeg(ENG_Quaternion q) {
        return toAngleAxisDeg(q, "");
    }

    public static String toAngleAxisDeg(ENG_Quaternion q, String prefix) {
        ENG_Vector3D axis = new ENG_Vector3D();
        float angle = q.toAngleAxisDeg(axis);
        return prefix + " axis: " + axis + " angle: " + angle;
    }

    /**
     * Hack to get around the thin/fat monitor stuff from RoboVM.
     * Used to generate the full monitor. This should solve the deadlock
     * happening in kryonet.
     * https://github.com/MobiVM/robovm/blob/master/compiler/vm/core/src/monitor.c
     * Search for LW_SHAPE_THIN.
     * @param o
     */
    public static void inflateMonitorObject(Object o) {
