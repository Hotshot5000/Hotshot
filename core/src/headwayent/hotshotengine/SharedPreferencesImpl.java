...
import headwayent.hotshotengine.android.app.QueuedWork;
...
//            ENG_Log.getInstance().log("Attempt to read preferences file " + mFile + " without permission", ENG_Log.TYPE_WARNING);
...
//                    ENG_Log.getInstance().log("getSharedPreferences: " + e.);
...
//        }
        ...
        if (map != null) {
            mMap = map;
            mStatTimestamp = mFile.lastModified();//stat.st_mtime;
            mStatSize = mFile.length();//stat.st_size;
        } else {
            mMap = new HashMap<>();
        }
        notifyAll();
...
//        }
        ...
        synchronized (this) {
            return mStatTimestamp != mFile.lastModified()/*stat.st_mtime*/ || mStatSize != mFile.length()/*stat.st_size*/;
        }
    }
    ...
    public final class EditorImpl implements Editor {
        ...
        private void notifyListeners(final MemoryCommitResult mcr) {
            ...
            if (Looper.myLooper() == Looper.getMainLooper()) {
                ...
                new Handler(Looper.getMainLooper()).post(() -> notifyListeners(mcr));
